...
Before submitting anything, run the safe authoring path first:
...
- Expected prerequisites: any machine for `validate` and `inspect`; a Linux Slurm login node plus the selected runtime backend for `up`
...
| --- | --- | --- |
| Authoring only | `minimal-batch.yaml` with `validate`, `inspect`, and `up --dry-run --skip-prepare --no-preflight` | Confirms the tool understands a spec without touching Slurm. |
