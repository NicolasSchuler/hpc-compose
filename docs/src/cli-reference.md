...
| `HPC008` | warning | `nodes`, `ntasks`, and `ntasks_per_node` are all set but `nodes * ntasks_per_node != ntasks`. Slurm resolves the mismatch by its own precedence, so the allocation may not be what the author intended. Checked at both the top level and per service. | Set `ntasks = nodes * ntasks_per_node`, or drop one of the three so the geometry is unambiguous. |
| `HPC009` | warning | `x-slurm.mem` or `mem_per_gpu` is a bare number with no unit suffix. Slurm reads an unsuffixed memory request as megabytes (MB), which is a frequent surprise. Checked at both the top level and per service. | Add an explicit unit suffix (e.g. `4G`, `512M`). |
...
| `experiment` | Track one run: aggregate, bundle, tag, and annotate | Parent command; `experiment show` aggregates a single run into one object, `experiment bundle` writes a local evidence bundle, and `experiment tag`/`experiment note` attach labels and observations to its tracked record. |
| `experiment show` | Aggregate one tracked run into a single read-only object | Combines scheduler status, the post-run efficiency score, the artifact manifest, and submit-time provenance into one object. `--format json` emits `{job_id, name, state, services[], provenance, results, efficiency, tags, notes, next_commands}` (`tags`/`notes` omitted when empty); each service carries `{name, nodelist, status, tunnel_hint}` with an `ssh -L` `ControlMaster` hint for TCP/HTTP readiness. Defaults to the latest tracked run; energy flags (`--pue`, `--gpu-tdp-w`, `--cpu-watts-per-core`) tune the embedded efficiency report. Static-safe: contacts the scheduler only as much as `status`/`score` do, writes nothing, and opens no connection. Example: `hpc-compose experiment show 12345 --format json`. |
| `experiment bundle` | Write a local reproducibility bundle for one tracked run | Creates `<into>/hpc-compose-bundle-<job-id>/` with `manifest.json`, `README.md`, `methods.md`, `run/experiment-show.json`, `run/submission-record.json`, `run/checkpoint-history.json`, and, when available, `run/effective-config.yaml`, `run/submitted.sbatch`, `provenance/provenance.json`, and `artifacts/manifest.json`. Add `--tarball` for `<into>/hpc-compose-bundle-<job-id>.tar.gz`. Artifact payload is metadata-only by default; add `--include-artifacts` and optional repeatable `--bundle <NAME>` filters to copy selected payload under `artifacts/payload/`. The bundle does not copy the current compose file as submit-time source; prefer the recorded config snapshot, submitted script, provenance, and source hash when present. `--format json` emits the same object written to bundle `manifest.json`: `{schema_version, job_id, created_at_unix, bundle_root, tarball_path, artifact_payload_included, selected_bundles, files[], warnings[]}`. Example: `hpc-compose experiment bundle 12345 --into ./bundles --tarball`. |
| `experiment tag` | Add or remove short labels on a tracked run | Positional tags are added; repeatable `--remove <TAG>` removes. Tags are a sorted set (`[A-Za-z0-9._-]`, max 64 chars, 32 per record); adding an existing tag or removing an absent one is a no-op. Defaults to the latest tracked run; `--job-id` targets an older one without ever repointing the "latest" record. Filter with `jobs list --tag`. `--format json` emits `{schema_version, job_id, tags}`. Contacts no scheduler. Example: `hpc-compose experiment tag baseline --job-id 12345`. |
| `experiment note` | Append a timestamped note to a tracked run | Appends one trimmed, non-empty observation (max 4096 chars) to the tracked record; notes are append-only and shown by `experiment show`. Defaults to the latest tracked run; `--job-id` targets an older one. `--format json` emits `{schema_version, job_id, notes[]}` where each note is `{text, created_at}`. Contacts no scheduler. Example: `hpc-compose experiment note 'diverged after epoch 3' --job-id 12345`. |
