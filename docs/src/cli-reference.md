...
| `experiment` | Track one run: aggregate, tag, and annotate | Parent command; `experiment show` aggregates a single run into one object, `experiment tag`/`experiment note` attach labels and observations to its tracked record. |
