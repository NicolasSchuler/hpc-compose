...
| `experiment` | Track one run: aggregate, tag, annotate, and bundle | Parent command; `experiment show` aggregates a single run into one object, `experiment tag`/`experiment note` attach labels and observations to its tracked record, and `experiment bundle` emits a reproducibility archive. |
...
| `experiment bundle` | Emit a paper-ready reproducibility archive for a tracked run | Bundles the compose spec, resolved config snapshot, rendered `sbatch`, full record, provenance (git SHA + dirty + image references), sweep manifest with seeds, metrics (`stats.csv` + raw `*.jsonl`), checkpoint history, and a generated `README.md` methods appendix, alongside a `MANIFEST.json` with per-file sha256 and a `missing[]` ledger. A `spec/spec-drift.diff` appears only when the current spec differs from the snapshot. Image entries are references as recorded at submit time, not content digests. Defaults to `experiment-bundle-<job_id>.tar.gz` (or `--output PATH`); `--dir` writes an unpacked directory. Missing ingredients degrade with a warning unless `--strict`, which fails after reporting. `--format json` is pinned by the `experiment-bundle` output schema. Defaults to the latest tracked run; contacts the scheduler only as much as `stats` does. Example: `hpc-compose experiment bundle 12345 --output run.tar.gz`. |
