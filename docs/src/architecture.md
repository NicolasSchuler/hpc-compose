...
- `domain`: privately own small pure shared policies for service identity, mounts, registries, rendezvous names, and human scheduler IDs
...
- `diagnostics::presentation`: privately own report text styling, per-thread notice formatting and stderr emission, and tracing setup
...
- `readiness_analysis`: privately own pure readiness locality classification and static TCP/HTTP endpoint derivation
...
- `job::file_digest`: privately own ordinary full-file SHA-256 streaming shared by artifact export and experiment bundles
...
- `time_util`: centralize crate-internal Unix-clock reads, system-time conversion, and shared duration constants
...
- `cache::dataset::store`: privately own mutable login-node staged-input CAS publication, completion records, legacy migration, and rebuildable tracking sidecars
...
- `commands::load`: share compose-to-plan/runtime-plan/effective-config orchestration between command families
...
3. `runtime_plan::build_runtime_plan` fingerprints each prepared local image immediately before pure service and cache-path derivation, then computes concrete cache artifact locations. The former `prepare::*` paths for this model remain compatibility re-exports.
...
10. `cli` parses typed command variants; the centralized `commands` router applies global policy and resolves shared context before family handlers execute; crate-private `output` formats the final presentation and owns checked output-schema registration.
