...
- `x-runtime.prepare.commands` installs `uv` once into the worker image,
...
- Cached image setup: `pip install --no-cache-dir uv` runs through `x-runtime.prepare.commands` and is reused until the worker image changes.
