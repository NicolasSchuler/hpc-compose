...
const EXAMPLES: &[ExampleInfo] = &[
    ...
    ExampleInfo {
        ...
        path: "examples/array-sweep-lite.yaml",
        availability: ExampleAvailability::RepositoryFile,
        category: "training",
        demonstrates: "Slurm job array via x-slurm.array with per-task SLURM_ARRAY_TASK_ID usage.",
        start_when: "You want N numbered independent tasks from one sbatch without a full sweep block.",
        tags: &["array", "slurm", "sweep", "batch"],
    },
    ExampleInfo {
        name: "notify-mail",
        path: "examples/notify-mail.yaml",
        availability: ExampleAvailability::RepositoryFile,
        category: "basics",
        demonstrates: "Slurm-sent start/end/fail email notifications via x-slurm.notify.email.",
        start_when: "You want the scheduler to mail you on job lifecycle events.",
        tags: &["notify", "mail", "email", "lifecycle"],
    },
    ExampleInfo {
        name: "healthcheck-compat",
        path: "examples/healthcheck-compat.yaml",
        availability: ExampleAvailability::RepositoryFile,
        category: "basics",
        demonstrates: "docker-compose healthcheck sugar normalized into readiness plus service_healthy gating.",
        start_when: "You are migrating a docker-compose healthcheck and want the supported nc/curl subset.",
        tags: &["healthcheck", "readiness", "compose-compat", "depends-on"],
    },
    ExampleInfo {
    ...
    ExampleInfo {
        ...
        path: "examples/deps-and-assert.yaml",
        availability: ExampleAvailability::RepositoryFile,
        category: "workflow",
        demonstrates: "Cross-job after_job dependency plus in-job assert with artifacts_contain.",
