...
pub use annotate::ProvenanceSpan;
pub use command::{
...
pub struct RenderOptions {
    ...
    /// Interleave provenance comments (`# <- x-slurm.mem` field markers and
    /// `# --- section (field) ---` banners) mapping rendered lines back to the
    /// spec fields that produced them. Preview-only: submission paths must
    /// keep this `false`, and with `false` the rendered script is
    /// byte-identical to an unannotated render.
    pub annotate: bool,
}
...
pub fn render_script_with_options(plan: &RuntimePlan, options: &RenderOptions) -> Result<String> {
    render_script_annotated(plan, options).map(|(script, _)| script)
}

/// Renders the script together with the provenance spans mapping instrumented
/// line ranges back to spec fields (the `explain` data source).
///
/// Coverage is deliberately partial: only the cleanly-mapped emit sites are
/// instrumented (SBATCH directives, feature-block sections, readiness gates,
/// and dependency waits). When `options.annotate` is `false` the returned
/// script is byte-identical to [`render_script_with_options`] output while the
/// spans still describe its line numbers.
pub fn render_script_annotated(
    plan: &RuntimePlan,
    ...
    let mut ann = Annotations::new(options.annotate);
    ...
    let job_name_source = if plan.slurm.job_name.is_some() {
        "x-slurm.job_name"
    } else {
        "name"
    };
    sbatch::push_directive(&mut out, &mut ann, job_name_source, "job-name", &plan.name);
    sbatch::push_directive(
        &mut out,
        &mut ann,
        "x-slurm.nodes",
        "nodes",
    ...
    if let Some(ntasks) = plan.slurm.ntasks {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.ntasks", "ntasks", ntasks);
    }
    ...
    if let Some(account) = &plan.slurm.account {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.account", "account", account);
    }
    if let Some(qos) = &plan.slurm.qos {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.qos", "qos", qos);
    }
    ...
    if let Some(licenses) = &plan.slurm.licenses {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.licenses", "licenses", licenses);
    }
    if let Some(mail_user) = plan.slurm.notify_email_recipient() {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.notify", "mail-user", mail_user);
    }
    if let Some(mail_type) = plan.slurm.notify_mail_type_value() {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.notify", "mail-type", mail_type);
    }
    if let Some(time) = &plan.slurm.time {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.time", "time", time);
    }
    ...
    if let Some(mem) = &plan.slurm.mem {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.mem", "mem", mem);
    }
    ...
    if let Some(gpu_bind) = &plan.slurm.gpu_bind {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.gpu_bind", "gpu-bind", gpu_bind);
    }
    if let Some(cpu_bind) = &plan.slurm.cpu_bind {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.cpu_bind", "cpu-bind", cpu_bind);
    }
    if let Some(mem_bind) = &plan.slurm.mem_bind {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.mem_bind", "mem-bind", mem_bind);
    }
    if let Some(distribution) = &plan.slurm.distribution {
        sbatch::push_directive(
            &mut out,
            &mut ann,
            "x-slurm.distribution",
            "distribution",
            distribution,
        );
    }
    if let Some(hint) = &plan.slurm.hint {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.hint", "hint", hint);
    }
    ...
    if let Some(output) = &plan.slurm.output {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.output", "output", output);
    ...
    if let Some(error) = &plan.slurm.error {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.error", "error", error);
    }
    if let Some(chdir) = &plan.slurm.chdir {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.chdir", "chdir", chdir);
    }
    if let Some(array) = &plan.slurm.array {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.array", "array", array);
    }
    if let Some(requeue) = plan.slurm.requeue {
        sbatch::push_bare_directive(
            &mut out,
            &mut ann,
            "x-slurm.requeue",
            if requeue { "requeue" } else { "no-requeue" },
        );
    }
    if let Some(signal_value) = plan.slurm.signal_directive_value() {
        sbatch::push_directive(&mut out, &mut ann, "x-slurm.signal", "signal", signal_value);
    }
    for arg in &plan.slurm.submit_args {
        sbatch::push_raw_directive(&mut out, &mut ann, "x-slurm.submit_args", arg);
    }
    if let Some(burst_buffer) = &plan.slurm.burst_buffer {
        ann.field(&mut out, "x-slurm.burst_buffer", |out| {
    ...
    if transfer_helpers_enabled {
        ...
        if scratch_enabled {
            stage_sources.push("x-slurm.scratch");
        }
        if !plan.slurm.stage_in.is_empty() {
            stage_sources.push("x-slurm.stage_in");
        }
        if !plan.slurm.stage_out.is_empty() {
            stage_sources.push("x-slurm.stage_out");
        }
        ann.section(&mut out, "stage/scratch helpers", &stage_sources, |out| {
            render_stage_helpers(out, plan);
        });
    }
    if hf_stage_enabled {
        ann.section(
            &mut out,
            "huggingface stage-in helpers",
            &["x-slurm.stage_in"],
            |out| render_hf_stage_in(out, plan, &options.huggingface_cli_bin),
        );
    }

    if metrics_enabled {
        ann.section(&mut out, "metrics helpers", &["x-slurm.metrics"], |out| {
            render_metrics_helpers(out);
        });
    }
    if software_env_enabled {
        ann.section(
            &mut out,
            "software env helpers",
            &["x-slurm.software_env"],
            render_software_env_helpers,
        );
    }
    ...
    if !plan.slurm.software_env.is_empty() {
        ann.section(
            ...
            "global software env",
            &["x-slurm.software_env"],
            |out| render_apply_software_env(out, &plan.slurm.software_env, ""),
        );
        out.push('\n');
    }
    if !plan.slurm.setup.is_empty() {
        ann.section(&mut out, "setup commands", &["x-slurm.setup"], |out| {
    ...
    for service in &plan.ordered_services {
        match readiness_source(service) {
            Some(source) => ann.field(&mut out, &source, |out| {
                render_readiness_wait(out, service);
            }),
            None => render_readiness_wait(&mut out, service),
        }
        out.push('\n');
    }
    ...
    for (service_index, service) in plan.ordered_services.iter().enumerate() {
        ...
        let service_source = format!("services.{}", service.name);
        ann.section(
            &mut out,
            &format!("service {}", service.name),
            &[service_source.as_str()],
            |out| render_service(out, service, &service_context),
        );
        out.push('\n');
    }

    for service in &plan.ordered_services {
        render_dependency_waits(&mut out, &mut ann, service);
    ...
    out.push_str("monitor_services\n");
    Ok((out, ann.into_spans()))
}

/// Spec path for a service's readiness gate, e.g.
/// `services.app.readiness.tcp`, or `None` when no readiness is configured.
fn readiness_source(service: &RuntimeService) -> Option<String> {
    let kind = match service.readiness.as_ref()? {
        ReadinessSpec::Sleep { .. } => "sleep",
        ReadinessSpec::Tcp { .. } => "tcp",
        ReadinessSpec::Log { .. } => "log",
        ReadinessSpec::Http { .. } => "http",
    };
    Some(format!("services.{}.readiness.{kind}", service.name))
}
...
fn render_dependency_waits(out: &mut String, ann: &mut Annotations, service: &RuntimeService) {
    for dependency in &service.depends_on {
        let source = format!(
            "services.{}.depends_on[{}].condition",
            service.name, dependency.name
        );
        ann.field(out, &source, |out| match dependency.condition {
